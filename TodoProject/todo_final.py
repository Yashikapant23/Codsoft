...
def sort_column(col):
    ...
    if col == "Due Date":
        sorted_data = sorted(tasks, key=lambda x: x["due_date"], reverse=reverse)
    elif col == "Priority":
        order = {"High":1, "Medium":2, "Low":3}
        sorted_data = sorted(tasks, key=lambda x: order.get(x["priority"],4), reverse=reverse)
    else:
        sorted_data = sorted(tasks, key=lambda x: x[col.lower()], reverse=reverse)
    refresh_list(sorted_data)
    ...
    if file:
        try:
            with open(file, "w", newline="") as f:
                writer = csv.writer(f)
                writer.writerow(["Task", "Status", "Due Date", "Priority"])
                for t in tasks:
                    status = "Done" if t["done"] else "Pending"
                    writer.writerow([t["task"], status, t["due_date"], t["priority"]])
            messagebox.showinfo("Export", f"Tasks exported to {file}")
...
export_button.grid(row=6, column=0, columnspan=2, pady=5)
...
for col in columns:
    task_tree.heading(col, text=col, command=lambda c=col: sort_column(c))

task_tree.grid(row=9, column=0, columnspan=2, pady=10)
